...
public class ChatActivity extends XBaseActivity implements 
	...
	protected void onInit(){
		...
		addAndManageEventListener(EventCode.IM_UserInfoChanged);
		...
		bindEventListenerRunnable(EventCode.IM_UserInfoChanged, r);
		bindEventListenerRunnable(EventCode.DownloadChatVoice, r);
		bindEventListenerRunnable(EventCode.DownloadChatVideo, r);
		bindEventListenerRunnable(EventCode.DownloadChatVideoThumb, r);
		bindEventListenerRunnable(EventCode.DownloadChatFile, r);
		bindEventListenerRunnable(EventCode.DownloadLocation, r);
	}
	...
	protected void onInitEditViewSendPlugin(){
		if(mEditView != null){
			...
			for(int i = 0;i < size;++i){
				mEditView.addViewIdSendPlugin(IMGlobalSetting.mapEditViewBtnIdToSendPlugin.keyAt(i));
			}
	...
	public void onSendPlugin(SendPlugin sp) {
		if(sp != null){
			...
			if(sendType == SendPlugin.SENDTYPE_PHOTO_ALL){
			...
			}else if(sendType == SendPlugin.SENDTYPE_VIDEO_ALL){
				if(mChooseVideoDialogId == 0){
					mChooseVideoDialogId = generateDialogId();
				}
				showDialog(mChooseVideoDialogId);
			}else if(sendType == SendPlugin.SENDTYPE_VIDEO_CAMERA){
				launchCamera(true);
			}else if(sendType == SendPlugin.SENDTYPE_VIDEO_ALBUMS){
				launchVideoChoose();
			}else if(sendType == SendPlugin.SENDTYPE_FILE){
				...
				final int requestCode = sp.getLaunchActivityRequestCode();
				if(requestCode >= SendPlugin.REQUESTCODE_MIN){
					if(mMapRequestCodeToSendPlugin == null){
						mMapRequestCodeToSendPlugin = new SparseArray<SendPlugin>();
					}
					mMapRequestCodeToSendPlugin.put(requestCode, sp);
					sp.startActivityForResult(this, requestCode);
				}else{
					XMessage xm = sp.onCreateXMessage();
					if(xm != null){
						onNewMessageEdited(xm, true);
						saveAndSendMessage(xm);
						if(sp.isHideEditPullUpView()){
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode == RESULT_OK){
			if(requestCode == mRequestCodeChooseFile){
				...
				if(mMapRequestCodeToSendPlugin != null){
					final SendPlugin sp = mMapRequestCodeToSendPlugin.get(resultCode);
					if(sp != null){
						final XMessage xm = sp.onActivityResult(resultCode, requestCode, data);
						if(xm != null){
							requestSendMessage(xm, sp.isHideEditPullUpView());
						}
					}
				}
			}
	...
	protected void requestSendMessage(XMessage xm,boolean bHidePullUpView){
		onNewMessageEdited(xm, true);
		saveAndSendMessage(xm);
		if(bHidePullUpView){
			mEditView.hideAllPullUpView(true);
	...
	public void onEventRunEnd(Event event) {
		...
		}else if(nCode == EventCode.UploadChatVideo ||
				nCode == EventCode.UploadChatPhoto ||
				nCode == EventCode.UploadChatVoice ||
	...
	protected void onReceiveMessage(XMessage m){
		...
		if(m.getType() == XMessage.TYPE_PHOTO){
			...
			LocationMessageDownloadProcessor.getInstance().requestDownload(m, false);
		}
	}
		...
		if(!mMessageAdapter.isCheck()){
			if (nViewId == R.id.viewContent) {
				...
				if (nType == XMessage.TYPE_VOICE) {
					...
					onLocationContentViewClicked(m);
				}
				...
				if(m.isFromSelf()){
					...
					if(type == XMessage.TYPE_FILE){
						if(m.isFileDownloading()){
							...
							FileMessageDownloadProcessor.getInstance().requestDownload(m, false);
						}
					}else if(type == XMessage.TYPE_VIDEO){
						if(m.isVideoDownloading()){
							VideoMessageDownloadProcessor.getInstance().stopDownload(m, false);
						}else{
							VideoMessageDownloadProcessor.getInstance().requestDownload(m, false);
						}
					}
				}
				redrawMessage(m);
	...
	protected void onLocationContentViewClicked(XMessage xm){
		if(xm.isLocationFileExists()){
			
		}else{
			LocationMessageDownloadProcessor.getInstance().requestDownload(xm, false);
			redrawMessage(xm);
		}
	}
