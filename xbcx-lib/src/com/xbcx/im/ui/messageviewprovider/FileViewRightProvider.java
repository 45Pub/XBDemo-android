...
public class FileViewRightProvider extends FileViewLeftProvider {
	...
	public FileViewRightProvider(OnViewClickListener listener) {
		super(listener);
	}
	...
	protected void onUpdateView(CommonViewHolder viewHolder, XMessage m) {
		...
		if(m.isFileUploading()){
			...
			fViewHolder.mProgressBar.setProgress(m.getFileUploadPercentage());
			...
			fViewHolder.mProgressBar.setProgress(m.getFileDownloadPercentage());
			fViewHolder.mButton.setVisibility(View.VISIBLE);
			fViewHolder.mButton.setBackgroundResource(R.drawable.msg_btn_gray);
			fViewHolder.mButton.setText(R.string.cancel);
			fViewHolder.mButton.setTextColor(0xffa40000);
